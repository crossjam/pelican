...
def wp_md2fields(xml, wp_custpost=False):
    for val in wp2fields(xml, wp_custpost):
        yield val[:-1] + ("wp-md",)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="wpfile_md",
        help="Wordpress XML export, markdown entries",
    )
    parser.add_argument(
    ...
    if args.wp_attach and not input_type.startswith("wordpress"):
    ...
    elif input_type == "wordpress-md":
        fields = wp_md2fields(args.input, args.wp_custpost or False)
